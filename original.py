...
logger = logging.getLogger("TechnicalAnalysisApp")
...
tickers_input = st.sidebar.text_input("Enter Tickers (comma-separated):", "BTC, ETH, ADA")
...
if st.sidebar.button("Fetch Data"):
    ...
    crypto_symbols = ["BTC", "ETH", "LTC", "XRP", "BCH", "ADA", "DOT", "LINK", "BNB", "XLM", "DOGE", "UNI", "SOL"]
    
    for ticker in tickers:
        try:
            # Check if this is a cryptocurrency ticker and append -USD if needed
            yf_ticker = ticker
            if ticker in crypto_symbols and not ticker.endswith("-USD"):
            ...
            if not data.empty:
                ...
                st.warning(f"No data found for {ticker}. For cryptocurrencies, try using the '-USD' suffix if not automatically detected.")
        except Exception as e:
...
if "stock_data" in st.session_state and st.session_state["stock_data"]:
    ...
    def analyze_ticker(ticker, data):
        ...
        # Create comprehensive description for the LLM
        data_description = f"""
        ...
        # Log the actual data being sent to ensure it's correct
        logger.info(f"Data being sent to LLM for analysis - Current price for {ticker}: {formatted_price}")
    ...
    for i, ticker in enumerate(st.session_state["stock_data"]):
        ...
        overall_results.append({"Stock": ticker, "Recommendation": result.get("action", "N/A")})        # In each ticker-specific tab, display the chart and detailed justification
        with tabs[i + 1]:
            # Check if this is likely a cryptocurrency 
            crypto_symbols = ["BTC", "ETH", "LTC", "XRP", "BCH", "ADA", "DOT", "LINK", "BNB", "XLM", "DOGE", "UNI", "SOL"]
